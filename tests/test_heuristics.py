...
def test_cpm_trivial_values():
    ...
    assert  33.75 == 𝓗(C, 𝓟)  # The graph K_10 with the trivial partition has CPM 33.75 (better than the singleton partition)


def test_modularity_comparison_networkx():
    """
    This test compares our implementation of modularity with the one provided by the NetworkX library.
    """
    # This test uses the so-called Zachary’s Karate Club graph, which represents the social interactions in a 
    ...
    assert abs(olom - nlom) < PRECISION, "The modularity (our implementation) does not match between our and NetworkX' implementation of the leiden algorithm!"
    assert abs(olnm - nlnm) < PRECISION, "The modularity (NX' implementation) does not match between our and NetworkX' implementation of the leiden algorithm!"

    # With the seed of 1 chosen above, the communities calculated by us and NetworkX match:
    assert P.as_set() == Q.as_set(), "The communities calculated by our implementation does not match the ones calculated by NetworkX's implementation of the Leiden algorithm!"